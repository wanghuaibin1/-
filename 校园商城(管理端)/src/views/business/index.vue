            ...
            <div class="flex">
              <div class="w-1/2 h-[100px]">
                <el-image style="width: 6.25rem; height: 6.25rem" preview-src-list
                  src="http://www.javacg.icu:7742/img/6f5b5f08-a345-4551-abb4-62568b4b7f76.jpg" fit="cover">
                  <template #placeholder>
                    <div class="image-slot">Loading<span class="dot">...</span></div>
                  </template>
                </el-image>
              </div>
              <div class="w-1/2 h-[100px]  flex items-center justify-center">
                {{ props.row.userName }}
                <!-- <span class="mb-0  text-slate-950">{{ props.row.userId }}</span> -->
              </div>
            </div>
          </template>
        </el-table-column>
        <el-table-column prop="phoneNumber" label="手机号" >
          <!-- <template #default="scope">
            <div style="display: flex; align-items: center">
              ...
              <span class=" font-bold">{{ scope.row.phoneNumber }}</span>
            </div>
          </template> -->
        </el-table-column>
...
import { adminUserAPI, deleteadminAPI, adminNameAPI } from "/src/api/administrators"
...
const search = async () => {
  ...
  const { data: res } = await adminNameAPI({ name: formInline.name })
  tableData.list = []
  if (res.value === "") return
  tableData.list.push(res.value)
}
const resetting = () => {
  formInline.name = ''
...
const getAAAAA = async () => {
  const { data: res } = await adminUserAPI({ yema: currentPage4.value, count: pageSize4.value })
...
const qinChuBanJi = (row) => {
  ElMessageBox.confirm(
    ...
    .then(async () => {
      const { data: res } = await deleteadminAPI({
        id: row.userId
      })
      console.log(res);
